...
def getOptionData(_asset, _strike, exp_date, _type):
    ...
    if exp_date == 0:
        price_history = oldestOptions(option_information)
...
firstStrike = str(
    int(price_data[tickers[0]]["Close"][price_data[tickers[0]].index[-1]])
)
secondStrike = str(
    int(price_data[tickers[1]]["Close"][price_data[tickers[1]].index[-1]])
)

# For leveraged assets, here is the multiplier. Asset 2 is an "assetMultiple" of asset 1.
assetMultiple = 3

firstAssetData = getOptionData(tickers[0], firstStrike, 0, "call")
secondAssetData = getOptionData(tickers[1], secondStrike, 0, "call")
...
(
    ...
    graph_x_axis,
) = calcAndPlot(tickers[0], firstStrike, 0, beginAnalysis)
...
(
    ...
    graph_x_axis,
) = calcAndPlot(tickers[1], secondStrike, 0, beginAnalysis)
