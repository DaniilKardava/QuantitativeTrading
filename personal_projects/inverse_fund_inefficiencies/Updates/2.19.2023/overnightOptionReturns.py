...
token = "Bearer eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..FMTfS__YvOizGZg9Dm3KY0P2GjJYwLSQqiDCkcW3YFTR8r3uU7GTanrWMMm2G3y80H6xVDDVyvlLPQGx7_QlSu4mIV0QpU-IsNnzIkRuyotRUWhE4qNtAIT1NG8ujmA2apQvJ7nYTZN9s148BI8FMetJVYfF2oiwy4TORyJjV_CPRkIhresLUvbmJ4TCp1kw2Skwl9MQEufvTihP_TiR_sPTo1tjYeYXxsz6RtEBqQ5pSvzeSy9FJ8uZd_inHB8Puckjd6PWrUxfeykRwyRm7YsVYRbDf3vplI3uU_-OIyuT4TSews-ly8-wJbL8ttih0AkTtWmn_QTEFdRqZeeXQw"
request_header = {
...
def getOptionData(_asset, _strike, exp_date, _type):
    ...
    if exp_date == 0:
        ...
        print(_type + " Oldest Option Price History: " + str(price_history))
    else:
...
def calcAndPlot(_ticker, _strike, _exp, _start_analysis):
    ...
    _start_analysis = max(put_data.index[0], call_data.index[0])
    call_data = call_data.truncate(before=_start_analysis)
    put_data = put_data.truncate(before=_start_analysis)
    ...
    call_data["open_price"] = call_data["open_price"].astype("float64")
    put_data["open_price"] = put_data["open_price"].astype("float64")

    put_intra_changes = np.array(put_data["close_price"]) - np.array(
        put_data["open_price"]
    )
    put_night_changes = np.array(put_data["open_price"][1:]) - np.array(
        put_data["close_price"][:-1]
    )

    call_intra_changes = np.array(call_data["close_price"]) - np.array(
        call_data["open_price"]
    )
    call_night_changes = np.array(call_data["open_price"][1:]) - np.array(
        call_data["close_price"][:-1]
    )
    ...
    temporaryPriceData = price_data[_ticker][["Close", "Open"]].copy()
    print(temporaryPriceData)

    for date in dividends.index:
        temporaryPriceData["Close"].loc[date:] = (
            temporaryPriceData["Close"].loc[date:] + dividends[date]
        )
        temporaryPriceData["Open"].loc[date:] = (
            temporaryPriceData["Open"].loc[date:] + dividends[date]
        )

    ticker_price_changes = np.diff(
        np.array(temporaryPriceData["Close"][-(len(put_changes) + 1) :])
    )
    ticker_night_changes = np.array(
        temporaryPriceData["Open"][-(len(put_changes) + 1) :][1:]
    ) - np.array(temporaryPriceData["Close"][-(len(put_changes) + 1) :][:-1])
    ticker_intra_changes = np.array(
        temporaryPriceData["Close"][-(len(put_changes) + 1) :]
    ) - np.array(temporaryPriceData["Open"][-(len(put_changes) + 1) :])
    ...
    plt.plot(
        put_data.index[1:],
        np.cumsum((call_night_changes - put_night_changes) - ticker_night_changes),
        label="Options - " + _ticker + " Night",
    )
    plt.legend()
    plt.xlabel("Date")
    plt.ylabel("Dollar Change")
    ...
    plt.plot(
        put_data.index,
        np.cumsum(call_intra_changes),
        label="Options Call Intra",
    )
    plt.plot(
        put_data.index[1:],
        np.cumsum(call_night_changes),
        label="Options Call Night",
    )
    ...
    plt.plot(
        put_data.index,
        np.cumsum((call_intra_changes - put_intra_changes) - ticker_intra_changes),
        label="Options - " + _ticker + " Intraday",
    )
    plt.legend()
    plt.xlabel("Date")
    plt.ylabel("Dollar Change")
    ...
    plt.plot(
        put_data.index,
        np.cumsum(put_intra_changes),
        label="Options Put Intra",
    )
    plt.plot(
        put_data.index[1:],
        np.cumsum(put_night_changes),
        label="Options Put Night",
    )
    ...
    # Overlay put and call by part of day
    plt.plot(
        put_data.index[1:],
        np.cumsum(put_night_changes),
        label="Options Put Night",
...
(
    ...
    graph_x_axis,
    first_asset_put_intra_changes,
    first_asset_put_night_changes,
    first_asset_call_intra_changes,
    first_asset_call_intra_changes,
...
(
    ...
    graph_x_axis,
    second_asset_put_intra_changes,
    second_asset_put_night_changes,
    second_asset_call_intra_changes,
    second_asset_call_intra_changes,
