...
# Store the index and corresponding weekdays returns for both assets. 0 corresponds to Monday, 4 to Friday.
...
for index in stocks_data.index:
    ...
    weekday_close_returns[index.weekday()][2].append(index)

for weekday in range(5):
    plt.yscale("log")
    plt.plot(
        weekday_close_returns[weekday][2],
        np.cumprod(
