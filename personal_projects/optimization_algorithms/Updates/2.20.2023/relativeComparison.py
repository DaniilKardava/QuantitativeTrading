...
if not populateTrainData:
    ...
    with open("allTimeReturns.txt", "r") as file:
        text = file.read()
    allTimePortfolioReturns = json.loads(text)[0]
    allTimeSPYReturns = json.loads(text)[1]
...
for subsample in range(1, int(len(backtest.index) / measurementPeriod)):
    try:
        spyVolatility = statistics.pstdev(
            backtest["SPY"][
                measurementPeriod * (subsample - 1) : measurementPeriod * subsample
            ]
        )
        portfolioVolatility = statistics.pstdev(
            backtest["Optimal_Portfolio"][
                measurementPeriod * (subsample - 1) : measurementPeriod * subsample
            ]
        )

        testPortfolioVolatility.append(portfolioVolatility - spyVolatility)
    except:
        continue
    ...
    if populateTrainData:
        trainingRunningPortfolioVolatility.append(
            ...
            - statistics.pstdev(backtest["SPY"][0 : measurementPeriod * subsample])
        )
        runningVolatilityIndex.append(backtest.index[subsample * measurementPeriod])
    else:
        trainingRunningPortfolioVolatility.append(
            statistics.pstdev(
                allTimePortfolioReturns
                + list(backtest["Optimal_Portfolio"][0 : measurementPeriod * subsample])
            )
            - statistics.pstdev(
                allTimeSPYReturns
                + list(backtest["SPY"][0 : measurementPeriod * subsample])
            )
        )
        runningVolatilityIndex.append(backtest.index[subsample * measurementPeriod])

        testRunningPortfolioVolatility.append(
            statistics.pstdev(
                backtest["Optimal_Portfolio"][0 : measurementPeriod * subsample]
            )
            - statistics.pstdev(backtest["SPY"][0 : measurementPeriod * subsample])
...
plt.title("Portfolio Volatility - SPY Volatility: TRAIN VS OOS")
plt.axvline(datetime.datetime(2019, 1, 1), label="OOS Begins")
plt.legend()
plt.show()

# Plot the difference in volatility between SPY and portfolio for just the test session
plt.plot(
    runningVolatilityIndex[-(len(testRunningPortfolioVolatility)) :],
    testRunningPortfolioVolatility,
)
plt.title("Portfolio Volatility - SPY Volatility: OOS ONLY")
plt.axvline(datetime.datetime(2019, 1, 1), label="OOS Begins")
plt.legend()
plt.show()
...
if not populateTrainData:
    ...
    # Compare variances of the portfolio train returns and the portfolio test returns across 2 diff time periods still makes no sense
    print("LEVENES, two sided")
...
if populateTrainData:
    with open("portfolioVolatility.txt", "w") as f:
        json.dump(trainingPortfolioVolatility, f)
