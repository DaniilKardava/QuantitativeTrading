...
while analysisYear <= 2022:
    ...
    ticker_data_percent = createData(tickers, analysisYear, outSample, 10)
    ...
    try:
        savedSolutions[analysisYear].append(solution)
    except:
        savedSolutions[analysisYear] = [solution]
...
def loadStratified():
    with open("../annualSharpeDevs/stratifiedInSharpes.txt", "r") as f:
        stratifiedInSharpes = json.load(f)
    with open("../annualSharpeDevs/stratifiedOutSharpes.txt", "r") as f:
        stratifiedOutSharpes = json.load(f)
    with open("../annualSharpeDevs/stratifiedEqualSharpes.txt", "r") as f:
...
if os.path.isfile("../annualSharpeDevs/stratifiedEqualSharpes.txt"):
    ...
    while year <= 2022:
        stratifiedDictionary[year] = []
        year += 1

    with open("../annualSharpeDevs/stratifiedEqualSharpes.txt", "w") as f:
        json.dump(stratifiedDictionary, f)
    with open("../annualSharpeDevs/stratifiedInSharpes.txt", "w") as f:
        json.dump(stratifiedDictionary, f)
    with open("../annualSharpeDevs/stratifiedOutSharpes.txt", "w") as f:
        json.dump(stratifiedDictionary, f)

    stratifiedInSharpes, stratifiedOutSharpes, stratifiedEqualSharpes = loadStratified()

stratifiedEqualSharpes = {int(k): v for k, v in stratifiedEqualSharpes.items()}
stratifiedOutSharpes = {int(k): v for k, v in stratifiedOutSharpes.items()}
stratifiedInSharpes = {int(k): v for k, v in stratifiedInSharpes.items()}
...
with open("../annualSharpeDevs/inSharpes.txt", "r") as f:
    savedInSharpes = json.load(f)
with open("../annualSharpeDevs/outSharpes.txt", "r") as f:
    savedOutSharpes = json.load(f)
with open("../annualSharpeDevs/equalSharpes.txt", "r") as f:
...
with open("../annualSharpeDevs/inSharpes.txt", "w") as f:
    json.dump(globalInSample, f)

with open("../annualSharpeDevs/outSharpes.txt", "w") as f:
    json.dump(globalOutSample, f)

with open("../annualSharpeDevs/equalSharpes.txt", "w") as f:
    json.dump(globalEqualSample, f)

# Save historic stratified sharpes
with open("../annualSharpeDevs/stratifiedEqualSharpes.txt", "w") as f:
    json.dump(stratifiedEqualSharpes, f)

with open("../annualSharpeDevs/stratifiedInSharpes.txt", "w") as f:
    json.dump(stratifiedInSharpes, f)

with open("../annualSharpeDevs/stratifiedOutSharpes.txt", "w") as f:
    json.dump(stratifiedOutSharpes, f)
...
for dateRange in list(stratifiedEqualSharpes.keys()):
    ...
    sharpeDiffs = np.array(stratifiedOutSharpes[dateRange]) - np.array(
        stratifiedEqualSharpes[dateRange]
    )
    means.append(statistics.mean(sharpeDiffs))

plt.scatter(list(stratifiedEqualSharpes.keys()), means)
