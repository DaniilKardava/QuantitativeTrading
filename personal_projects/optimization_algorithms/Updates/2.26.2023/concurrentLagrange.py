...
for variable in equation_variables[:-1]:
...
def solve_equation():
    global counter
    guess = list(random.uniform(0, 25) for a in range(length_of_assets))
    guess.append(0)
    solution = sympy.nsolve(
        system_of_equations, equation_variables, guess, verify=False
    )
    subs = {}
...
with ThreadPoolExecutor(max_workers=10) as executor:
    futures = [executor.submit(solve_equation) for n in range(10)]
